#!binbash
helptext=Usage  $(basename $0) [Options]...
...
snap_create () { #create snapshot
	...
	snap_id=($(xe vm-snapshot vm=${vm_array[i]} new-name-label=${vm_array[i]}@$tag@$date));
	if [ $ -ne 0 ]; then
	    backup_failed;
		errors 4;
	fi
...
snap_export () { #export snapshot
	...
	xe vm-export vm=$snap_id filename=varrunsr-mount$NFS${vm_array[i]}@$tag@$date;
	if [ $ -ne 0 ]; then
			backup_failed;
			errors 5;
	fi	
...
More information can be found in .varlogbackup.sh
END_MAIL
fi
}
...
vm_array=($(xe vm-list tagscontains=$tag grep label cut -c24-));
...
do
	...
	vm_state=($(xe vm-list uuid=$uuidgrep power-state ( RO) runningwc -l));
	...
	if [ $vm_state == 1 ]; then
			...
			if [ $snap_count -gt $snap_keep ]; then
					...
					snap_array=($(xe snapshot-list snapshot-of=$uuidgrep name-labelgrep ${vm_array[i]}@$tag@ cut -d @ -f 3));
...
send_mail Backup report  ${#vm_array_complete[@]} VMs successfully backed up vm_array_complete[@]  ${#vm_array_off[@]} VMs were skipped due to being powered off vm_array_off[@]
